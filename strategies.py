...
def choose_literal_majority_rule(clauses):
    """
    Choose a variable based on the maximum difference between
    its number of positive and negative occurrences.
    """
    pos_count = Counter()
    neg_count = Counter()
    ...
    # Compute difference between positive and negative counts
    max_diff = -1
    chosen_var = None
    for var in set(pos_count.keys()).union(neg_count.keys()):
        diff = abs(pos_count[var] - neg_count[var])
        if diff > max_diff:
            max_diff = diff
            # Choose the literal with more occurrences (positive or negative)
            chosen_var = var if pos_count[var] >= neg_count[var] else -var
