...
def dpll(clauses, splitting_strategy="first", depth=0, msg=True, nr_splits=0):
    ...
    elif splitting_strategy == "majority_rule":
        literal = choose_literal_majority_rule(clauses)
    else:
...
def apply_dpll(clauses, msg=True):
    ...
    if msg:
        ...
        for i, clause in enumerate(clauses, 1):
            print(f"({i}) {format_clause(clause)}")
        print("-" * 60)
    result, nr_splits= dpll(clauses, splitting_strategy="random", msg=False)# Try "random", "first", "most_frequent",
                                                                                        # "MOMS", "MAMS", "JW", "DLCS" or "majority_rule"
    if result:
        print("The formula is SATISFIABLE")
        print("Time measurement: ", time.time() - start_time)
    else:
